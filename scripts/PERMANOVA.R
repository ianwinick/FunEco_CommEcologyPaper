...
# QUGA seedmass is changed to NA in the trait table, QUGA resprouting put back in
...
# binary 0,1 doesnt work for some reason in the dbFD function (or functcomp) so recode the traits
...
################################################################################
################################################################################
# CWM calculated using functcomp, which allegedly excludes NAs, instead of dbFD (good call, Fern!)
################################################################################
################################################################################
cwm <- functcomp(traits, comm)

# Put resprouting between 0 and 1, rather than between 1 and 2
cwm$resprouting <- cwm$resprouting-1
...
# pair_fun <- pairwise.adonis(vegdist(cwm, method="euclidean"), data$Severity, perm=9999)
# 
# # Test of beta dispersion and post-hoc pairwise test of beta dispersion
